...
public class DynamicIntArrayImprove<T> implements StringList<Integer> {
    private final int DEFAULT_SIZE_ARRAY = 10;
    private int size;

    private Integer[] internalArray;

    public DynamicIntArrayImprove() {
        internalArray = new Integer[DEFAULT_SIZE_ARRAY];
    }

    public DynamicIntArrayImprove(int initialSize) throws IllegalParamExeption {
        if (initialSize > 0) {
    ...
    public DynamicIntArrayImprove(Collection<String> initialArray) {
    ...
    public void grow() {
        if (internalArray.length - size <= 1) {
            Integer[] newSize = new Integer[internalArray.length + internalArray.length / 2];
    ...
    public boolean contains(Integer item) {
        quickSort(internalArray, 0, internalArray.length - 1);
